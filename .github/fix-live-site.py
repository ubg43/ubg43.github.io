...
(function(){
  ...
  function key(c){var h=c.querySelector('h3'),i=c.querySelector('img');return norm(h?h.textContent:(i?i.alt:''))+'|'+(i?i.getAttribute('src'):'')}
  function decorate(){
    var cs=cards(); if(!cs.length)return;
    cs.forEach(function(c){
      var old=c.querySelector('.card-ribbons');if(old)old.remove();
      ...
      if(c.dataset.trendingSeed==='true'){var t=document.createElement('span');t.className='card-ribbon trending';t.textContent='TRENDING';box.appendChild(t)}
...
def patch_index(text):
    ...
    # Keep the already-rendered cards visible even when the legacy mirror cannot be fetched.
