...
function init(){wireCategories();wireScroll();interceptGameClicks();new MutationObserver(()=>{clearTimeout(renderTimer);renderTimer=setTimeout(()=>{renderCategories();wireScroll();apply()},120)}).observe(grid,{childList:true});apply();wireScroll()}
