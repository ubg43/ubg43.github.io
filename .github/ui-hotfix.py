...
function trendKeys(){try{const h=JSON.parse(localStorage.getItem('ubg43_v3_plays')||'{}');return new Set(cards().map(c=>{const g=info(c),k=norm(g.title)+'|'+(c.querySelector('img')?.getAttribute('src')||'');const x=h[k]||{};return{key:k,score:(x.plays||0)+((Date.now()-(x.last||0)<259200000)?2:0)}}).sort((a,b)=>b.score-a.score||a.key.localeCompare(b.key)).slice(0,24).map(x=>x.key))}catch(_){return new Set()}}
...
const runNext=()=>{while(running<8&&queue.length){const card=queue.shift();running++;const a=card.getAttribute('onclick')||'',m=a.match(/openGame\(\s*['"]([^'"]+)['"]\s*\)/),url=m?m[1]:'';if(!url){running--;card.remove();continue}fetch(url,{method:'HEAD',cache:'no-store',redirect:'follow'}).then(r=>{if(!r.ok&&r.status!==0)card.remove()}).catch(()=>{}).finally(()=>{running--;runNext()})}};
const inspect=()=>{[...grid.querySelectorAll(':scope > .game-card')].forEach(card=>{if(checked.has(card))return;checked.add(card);const url=(card.getAttribute('onclick')||'');if(url.includes('raw.githubusercontent.com/gn-math/html/main/'))queue.push(card)});runNext()};
new MutationObserver(inspect).observe(grid,{childList:true});inspect();
})();</script>'''

def patch(text):
    # Remove the older duplicate hotfix blocks so this script stays the single compatibility layer.
    ...
    # Remove the two original empty featured rails; V3 renders the populated rails in their place.
