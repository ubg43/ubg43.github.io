...
@media(max-width:600px){.v3-strip{padding-left:14px!important;padding-right:14px!important}.v3-carousel{grid-auto-columns:calc((100vw - 40px)/2)!important;min-height:210px!important}#status{margin-left:14px!important;margin-right:14px!important}}
</style>'''
...
function moveLayout(){
 const header=document.querySelector('.site-header')||document.querySelector('header'); if(header&&header.parentNode===document.body&&document.body.firstElementChild!==header)document.body.insertBefore(header,document.body.firstElementChild);
 const hero=document.querySelector('.hero')||document.getElementById('v3Hero');
 const main=grid.closest('main')||document.querySelector('main')||document.body;
 if(hero&&hero.parentNode!==main)main.insertBefore(hero,main.firstChild);
 ['v3Trending','v3New','v3Recommendations'].forEach(id=>{const s=$(id);if(!s)return;s.style.display=s.classList.contains('is-hidden')?'none':'block';if(s.parentNode!==main)main.insertBefore(s,grid);});
 document.querySelectorAll('section.featured').forEach(s=>s.remove());
}
...
function normalize(){moveLayout();decorateLegacy();normalizeRail('v3Trending','trend');normalizeRail('v3New','new');normalizeRail('v3Recommendations','trend');syncStatus();const native=search?.closest('.search-shell')?.querySelector('input[type=search]::-webkit-search-cancel-button');void native;}
...
if(search){search.addEventListener('input',()=>setTimeout(syncStatus,30));search.addEventListener('change',()=>setTimeout(syncStatus,30));search.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();e.stopPropagation();setTimeout(()=>grid.scrollIntoView({behavior:'smooth',block:'start'}),50)}} ,true)}
if(clear)clear.addEventListener('click',()=>setTimeout(syncStatus,30),true);
new MutationObserver(()=>{normalize()}).observe(document.body,{childList:true,subtree:true});
window.addEventListener('resize',()=>{normalizeRail('v3Trending','trend');normalizeRail('v3New','new');normalizeRail('v3Recommendations','trend')});
setTimeout(normalize,50);setTimeout(normalize,400);setTimeout(normalize,1200);setTimeout(normalize,2500);
})();</script>'''

def patch(text):
    # Keep the verified GN-Math loader from adding obvious dead candidates.
    ...
    # Replace only this hotfix layer; leave the core V3 runtime intact.
    ...
    # Remove any old v3 duplicate rails so the runtime rebuild is the only source.
