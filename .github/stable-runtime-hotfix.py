...
/* Always-visible maintenance marker: does not depend on JavaScript. */
.game-card[data-maintenance="true"]{position:relative!important}
.game-card[data-maintenance="true"]::before{content:"MAINTENANCE";position:absolute!important;left:9px!important;top:9px!important;z-index:999!important;display:flex!important;align-items:center!important;justify-content:center!important;height:22px!important;padding:0 10px!important;border-radius:6px 8px 8px 6px!important;background:#f59e0b!important;color:#fff!important;font:900 9px/1 Arial,sans-serif!important;letter-spacing:.06em!important;box-shadow:0 4px 10px rgba(0,0,0,.28)!important;white-space:nowrap!important;pointer-events:none!important;opacity:1!important;visibility:visible!important}
...
.ubg43-badge.new{background:#e52424;color:#ffe600}.ubg43-badge.trending{background:#ffe600;color:#c31d1d}.ubg43-badge.maintenance{background:#f59e0b;color:#fff}
...
.sidebar{transform:translate3d(105%,0,0);transition:transform .34s cubic-bezier(.22,.9,.28,1)!important;will-change:transform}.sidebar.open{transform:translate3d(0,0,0)!important}.overlay{display:block!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important;transition:opacity .28s ease,visibility 0s linear .28s!important;backdrop-filter:blur(3px);-webkit-backdrop-filter:blur(3px)}.overlay.open{visibility:visible!important;opacity:1!important;pointer-events:auto!important;transition:opacity .28s ease,visibility 0s linear 0s!important}.sidebar.open .category{animation:ubg43-category-in .38s cubic-bezier(.2,.8,.25,1) both;animation-delay:var(--ubg43-category-delay,0ms)}@keyframes ubg43-category-in{from{opacity:0;transform:translate3d(22px,0,0)}to{opacity:1;transform:translate3d(0,0,0)}}.category{will-change:transform,opacity}@media(prefers-reduced-motion:reduce){.sidebar,.overlay,.category{transition:none!important}.sidebar.open .category{animation:none!important}}
...
.ubg43-searching .hero{display:none}.ubg43-searching #searchPage{display:block!important}.ubg43-searching #gameGrid{padding-top:6px}.ubg43-searching #trendingSection,.ubg43-searching #newSection{display:none!important}.ubg43-category-view #trendingSection,.ubg43-category-view #newSection{display:none!important}
...
const rawSearch=s=>String(s??'').normalize('NFKC').replace(/[\\u200B-\\u200D\\uFEFF]/g,'').replace(/\\s+/g,' ').trim().toLowerCase();
const isExactSymbolQuery=s=>/^[^a-z0-9\\s][a-z0-9]{1,2}$/i.test(rawSearch(s));
const searchMatches=(title,query)=>{
  const q=rawSearch(query);
  if(!q)return true;
  const t=rawSearch(title);
  if(t===q)return true;
  if(isExactSymbolQuery(q))return false;
  ...
  return queryWords.length>1&&queryWords.every(w=>titleWords.some(tw=>tw.startsWith(w)));
};
...
const updateTrendRailState=()=>{const sub=document.querySelector('#trendingSection .section-sub');if(!sub)return;const live=Object.keys(globalTrending.counts).length;sub.textContent=live?'Most played across all UBG43':globalTrending.loading?'Trending picks • loading global play activity…':globalTrending.failed?'Trending picks • will update automatically':'Popular picks based on play activity'};
...
const adPattern=new RegExp(AD_HOST_PATTERNS.concat(AD_HTML_PATTERNS).map(x=>x.replace(/[.*+?^{}()|[\]\\]/g,'\\runtime = r'''<style id="ubg43-final-runtime-style">
.ubg43-badges{position:absolute;left:9px;top:9px;z-index:20;display:flex;flex-direction:column;gap:5px;pointer-events:none}
.ubg43-badge{display:inline-flex;align-items:center;height:22px;padding:0 9px;border-radius:6px 8px 8px 6px;font:900 9px/1 Arial,sans-serif;letter-spacing:.06em;box-shadow:0 4px 10px rgba(0,0,0,.22);white-space:nowrap}
...
function handleImageError(img){
  ...
  let step=Number(img.dataset.ubg43ImageStep||0);
  const source=img.dataset.ubg43ImageSource||img.getAttribute('src')||'';
  if(step<alternates.length){
    const next=alternates[step++];
    img.dataset.ubg43ImageStep=String(step);
    img.src=next;
    return;
  }
...
function wireImage(img){
  ...
  if(img.dataset.ubg43ImageWired==='1')return;
  img.dataset.ubg43ImageWired='1';
  if(!img.dataset.ubg43ImageSource)img.dataset.ubg43ImageSource=img.getAttribute('src')||'';
  img.addEventListener('error',()=>handleImageError(img),{passive:true});
  if(img.complete&&img.naturalWidth===0&&img.getAttribute('src'))setTimeout(()=>handleImageError(img),0);
}
function repairImages(){cards().forEach(c=>c.querySelectorAll?.('img').forEach(wireImage))}
document.addEventListener('error',e=>{const el=e.target;if(el?.tagName==='IMG')handleImageError(el)},true);
function wire(c){if(!c)return;c.tabIndex=0;if(!c.dataset.url)c.dataset.url=urlOf(c);c.querySelectorAll?.('img').forEach(wireImage)}
...
function recommendations(q){
  ...
  const cs=cards(),n=norm(q);
  if(!n){recSection.classList.add('hidden');recRail.innerHTML='';return}
  const ph=read('ubg43_final_plays',{}),sh=read('ubg43_final_searches',{});
  const searchHits=cs.filter(c=>norm(titleOf(c)).includes(n)&&!blockedTitle(c));
  const catAffinity=Object.create(null),playAffinity=Object.create(null);
  searchHits.forEach(c=>{const cat=String(c.dataset.category||'').trim();if(cat)catAffinity[cat]=(catAffinity[cat]||0)+1});
  Object.entries(ph).forEach(([k,v])=>{
    const plays=Math.min(20,Number(v?.plays||0));
    if(plays<=0)return;
    cs.forEach(c=>{
      const sim=similarity(titleOf(c),String(v?.title||k));
      if(sim>0)playAffinity[keyOf(c)]=(playAffinity[keyOf(c)]||0)+sim*Math.log2(plays+1);
    });
  });
  const out=cs.filter(c=>{
    const t=norm(titleOf(c));
    return t&&!t.includes(n)&&!blockedTitle(c);
  }).map(c=>{
    const title=titleOf(c),t=norm(title),cat=String(c.dataset.category||'').trim();
    let score=0;
    const direct=similarity(title,q);
    score+=direct*1.15;
    const qTokens=new Set(n.split(' ').filter(x=>x.length>1));
    const tTokens=new Set(t.split(' ').filter(x=>x.length>1));
    let shared=0;qTokens.forEach(x=>{if(tTokens.has(x))shared++});
    if(shared)score+=Math.min(.72,shared*.18);
    if(cat&&catAffinity[cat])score+=Math.min(.9,catAffinity[cat]*.16);
    score+=(playAffinity[keyOf(c)]||0)*.16;
    Object.entries(sh).forEach(([k,v])=>{
      const sv=similarity(title,k),count=Math.min(10,Number(v?.count||0));
      if(sv>0)score+=sv*(.12+Math.log2(count+1)*.04);
    });
    if(isNew(c))score+=.04;
    if(isTrending(c))score+=.025;
    return {c,score};
  }).sort((a,b)=>b.score-a.score||titleOf(a.c).localeCompare(titleOf(b.c))).slice(0,24).map(x=>x.c);
  fillRail(recRail,out);
  recSection.classList.toggle('hidden',!out.length);
}
function setSearchMode(q){q=q.trim();if(!q){clearSearch();return}window.__ubg43SearchMode=true;document.body.classList.add('ubg43-searching');$('trendingSection')?.style.setProperty('display','none','important');$('newSection')?.style.setProperty('display','none','important');if(searchPage)searchPage.classList.remove('hidden');if(searchPageText)searchPageText.textContent=`Showing matching games for “${q}”.`;recordSearch(q);applyView();recommendations(q);results?.classList.remove('open');search?.blur()}
function clearSearch(){window.__ubg43SearchMode=false;document.body.classList.remove('ubg43-searching');if(search)search.value='';if(clear)clear.style.display='none';results?.classList.remove('open');searchPage?.classList.add('hidden');recSection?.classList.add('hidden');$('trendingSection')?.style.removeProperty('display');$('newSection')?.style.removeProperty('display');applyView();window.scrollTo({top:0,behavior:'smooth'})}
...
function openCategories(){const side=$('categorySidebar'),ov=$('categoryOverlay');if(!side)return;side.classList.add('open');ov?.classList.add('open');side.setAttribute('aria-hidden','false');document.body.classList.add('locked');const list=$('categoryList');if(!list)return;list.innerHTML='';const cs=cards(),counts={};cs.forEach(c=>{const cat=c.dataset.category||'Casual';counts[cat]=(counts[cat]||0)+1});['All Games','New Games','Trending','Action','Adventure','Horror','Multiplayer','Fighting','Survival','Platformer','Racing','Sports','Puzzle','Arcade','Strategy','Simulation','Casual','Anime','Rhythm & Music','Card & Board','io'].forEach((cat,index)=>{const b=document.createElement('button');b.type='button';b.className='category'+(window.__ubg43ActiveCategory===cat?' active':'');const n=cat==='All Games'?cs.length:cat==='New Games'?cs.filter(isNew).length:cat==='Trending'?cs.filter(isTrending).length:(counts[cat]||0);b.innerHTML='<span class="category-main"><span class="dot"></span><span class="category-name"></span></span><span class="category-count"></span>';b.querySelector('.category-name').textContent=cat;b.querySelector('.category-count').textContent=String(n);b.style.setProperty('--ubg43-category-delay',`${45+index*28}ms`);b.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();window.__ubg43ActiveCategory=cat;document.body.classList.toggle('ubg43-category-view',cat!=='All Games');applyView();closeCategories()},{capture:true});list.append(b)})}
...
function bind(){
 search?.addEventListener('input',e=>{e.stopImmediatePropagation();const q=search.value.trim();if(clear)clear.style.display=q?'inline-flex':'none';document.body.classList.toggle('ubg43-searching',!!q);showSuggestions(q);if(!q)clearSearch();else {applyView();recommendations(q)}},{capture:true});
...
function sync(){cards().forEach(c=>{if(blockedTitle(c))c.remove();else wire(c)});repairImages();decorate();renderRails();applyView();const s=$('status');if(s&&cards().length)s.textContent=cards().length+' games ready'}
...
async function mountGame(w,u,title){
  ...
  iframe.referrerPolicy='no-referrer';
  iframe.allow='autoplay; fullscreen; gamepad; clipboard-read; clipboard-write';
  if(/^https:\/\/(?:ubg43\.github\.io\/games\/buckshot-roulette\.html|sites\.google\.com\/view\/classroom6x\/buckshot-roulette)(?:[/?#]|$)/i.test(u)){
    iframe.src=u;
    iframe.style.cssText='position:fixed;inset:0;width:100%;height:100%;border:0;display:block;background:#000';
    iframe.setAttribute('allowfullscreen','');
    iframe.setAttribute('allow','fullscreen; autoplay; gamepad; clipboard-read; clipboard-write');
    const fs=w.document.createElement('button');
    fs.type='button';
    fs.textContent='FULLSCREEN';
    fs.setAttribute('aria-label','Enter fullscreen');
    fs.style.cssText='position:fixed;right:14px;top:14px;z-index:2147483647;border:0;border-radius:10px;padding:10px 13px;background:rgba(6,20,47,.88);color:#fff;font:900 11px/1 Arial,sans-serif;letter-spacing:.06em;cursor:pointer;box-shadow:0 6px 18px rgba(0,0,0,.3)';
    fs.addEventListener('click',async()=>{
      try{await (iframe.requestFullscreen?.()||stage.requestFullscreen?.())}catch(_){}
...
window.openGame=openGame;
let cardCache=null;
function cards(){if(!grid)return [];if(!cardCache)cardCache=[...grid.querySelectorAll('.game-card')];return cardCache}
function invalidateCards(){cardCache=null}
async function loadLegacyIntoGrid(){
  if(cards().length>=300)return cards().length;
  try{
    const controller=new AbortController();const timer=setTimeout(()=>controller.abort(),7000);
    ...
    const source=[...d.querySelectorAll('.game-card')];
    if(source.length<100)throw new Error('legacy game count too low');
    grid.textContent=''; invalidateCards(); const seen=new Set();
    source.forEach(src=>{
      const h=src.querySelector('h3'),img=src.querySelector('img'); if(!h||!img)return;
      const rawTitle=h.textContent.trim();
      const title=rawTitle.toLowerCase().replace(/[^a-z0-9]+/g,' ').trim();
...
const fallbackTrendingKeys=new Set();
function isTrending(c){return globalCount(c)>0||fallbackTrendingKeys.has(keyOf(c))}
const MAINTENANCE_TITLE_KEYS=new Set(['steal a egg online','obby steal an egg online','meccha chameleon online']);
function isMaintenance(c){return c?.dataset?.maintenance==='1'||c?.dataset?.maintenance==='true'||MAINTENANCE_TITLE_KEYS.has(norm(titleOf(c)))}
function badge(c,text,cls){const box=c.querySelector('.ubg43-badges')||(()=>{const x=document.createElement('div');x.className='ubg43-badges';c.append(x);return x})();const b=document.createElement('span');b.className='ubg43-badge '+cls;b.textContent=text;box.append(b)}
function decorate(){const cs=cards();cs.forEach(c=>{c.querySelector('.ribbons')?.remove();c.querySelector('.ubg43-badges')?.remove();if(isNew(c))badge(c,'NEW','new');if(isTrending(c))badge(c,'TRENDING','trending')})}
const IMAGE_PROXY='https://images.weserv.nl/?url=';
const KNOWN_IMAGE_ALTERNATES={'blumgi dragon':['https://pokigamez.github.io/images/blumgi-dragon.png','https://retrobowl-25.github.io/thumbs/blumgi-dragon.webp','https://bitlifeonline.github.io/blumgi-dragon/logo.png']};
...
function wire(c){if(!c)return;c.tabIndex=0;if(!c.dataset.url)c.dataset.url=urlOf(c);c.querySelectorAll?.('img').forEach(wireImage)}
function applyView(){
  const q=search?.value||'',hasSearch=!!rawSearch(q),exact=isExactSymbolQuery(q),cat=window.__ubg43ActiveCategory||'All Games',cs=cards();
  cs.forEach(c=>{
    const catOk=hasSearch||exact||cat==='All Games'||(cat==='New Games'?isNew(c):cat==='Trending'?isTrending(c):(c.dataset.category||'Casual')===cat);
    c.style.display=catOk&&searchMatches(titleOf(c),q)?'':'none';
  });
  const shown=cs.filter(c=>c.style.display!=='none').length;
  if($('status'))$('status').textContent=shown+' games shown';
}
function similarity(a,b){const A=new Set(norm(a).split(' ').filter(x=>x.length>1)),B=new Set(norm(b).split(' ').filter(x=>x.length>1));if(!A.size||!B.size)return 0;let n=0;A.forEach(x=>B.has(x)&&n++);return n/Math.sqrt(A.size*B.size)}
function fillRail(rail,srcs){if(!rail)return;rail.innerHTML='';const used=new Set();srcs.forEach(src=>{if(blockedTitle(src))return;const k=keyOf(src);if(used.has(k))return;used.add(k);const c=src.cloneNode(true);c.dataset.url=urlOf(src);c.style.display='';c.hidden=false;c.querySelector('.ribbons')?.remove();c.querySelector('.ubg43-badges')?.remove();rail.append(c);wire(c);if(isNew(src))badge(c,'NEW','new');if(isTrending(src))badge(c,'TRENDING','trending')});if(srcs.length){const d=document.createElement('div');d.className='done';d.innerHTML='<span>That’s all for now ✨<small>More games are added automatically.</small></span>';rail.append(d)}}
function renderRails(){
...
function recommendations(q){
  if(!recSection||!recRail)return;
  const cs=cards(),n=norm(q);
  if(!n||isExactSymbolQuery(q)){recSection.classList.add('hidden');recRail.innerHTML='';return}
  const ph=read('ubg43_final_plays',{}),sh=read('ubg43_final_searches',{});
  const searchHits=cs.filter(c=>searchMatches(titleOf(c),q)&&!blockedTitle(c));
  const catAffinity=Object.create(null),playAffinity=Object.create(null);
  searchHits.forEach(c=>{const cat=String(c.dataset.category||'').trim();if(cat)catAffinity[cat]=(catAffinity[cat]||0)+1});
  Object.entries(ph).forEach(([k,v])=>{
    const plays=Math.min(20,Number(v?.plays||0));
...
function showSuggestions(q){
  ...
  results.innerHTML='';
  const n=norm(q);
  if(!n&&!isExactSymbolQuery(q)){results.classList.remove('open');return}
  const hits=cards().filter(c=>searchMatches(titleOf(c),q)).slice(0,7);
  if(!hits.length){
    const d=document.createElement('div');
    d.className='search-empty';
    d.textContent='No matching games yet';
    results.append(d);
  ...
  hits.forEach(c=>{
    ...
    b.innerHTML='<img alt=""><span class="search-copy"><span class="search-title"></span><span class="search-label">Play game</span></span>';
    b.querySelector('img').src=imageOf(c);
    b.querySelector('img').alt=titleOf(c);
    b.querySelector('.search-title').textContent=titleOf(c);
    b.addEventListener('click',e=>{
      e.preventDefault();
      e.stopImmediatePropagation();
      if(search)search.value=titleOf(c);
      setSearchMode(titleOf(c));
    },{capture:true});
    results.append(b);
  });
  results.classList.add('open');
...
function bind(){
 ...
 search?.addEventListener('input',e=>{e.stopImmediatePropagation();const q=search.value.trim();if(clear)clear.style.display=q?'inline-flex':'none';document.body.classList.toggle('ubg43-searching',!!q);if(searchTimer)clearTimeout(searchTimer);searchTimer=setTimeout(()=>{showSuggestions(q);if(!q)clearSearch();else{applyView();recommendations(q)}},80)},{capture:true});
 search?.addEventListener('keydown',e=>{e.stopImmediatePropagation();if(e.key==='Enter'){e.preventDefault();setSearchMode(search.value)}else if(e.key==='Escape'){e.preventDefault();clearSearch()}},{capture:true});
 clear?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();clearSearch()},{capture:true});
 $('randomGameButton')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const pool=cards().filter(c=>c.style.display!=='none'),list=pool.length?pool:cards(),c=list[Math.floor(Math.random()*list.length)];if(c){recordPlay(c);openGame(urlOf(c),titleOf(c));renderRails()}},{capture:true});
 $('reportGameButton')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();window.open(REPORT_URL,'_blank','noopener,noreferrer')},{capture:true});
 $('categoryToggle')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();openCategories()},{capture:true});
 $('categoryClose')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();closeCategories()},{capture:true});
 $('categoryOverlay')?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();closeCategories()},{capture:true});
 [['trendPrev','trendingRail'],['trendNext','trendingRail'],['newPrev','newRail'],['newNext','newRail'],['recPrev','recommendRail'],['recNext','recommendRail']].forEach(([id,rid])=>$(id)?.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();const r=$(rid);if(r)r.scrollBy({left:Math.max(280,r.clientWidth*.8)*(id.includes('Prev')?-1:1),behavior:'smooth'})},{capture:true}));
 ['trendingRail','newRail','recommendRail'].forEach(id=>$(id)?.addEventListener('wheel',e=>{if(Math.abs(e.deltaY)>Math.abs(e.deltaX)){e.currentTarget.scrollLeft+=e.deltaY}},{passive:true}));
 document.addEventListener('click',e=>{const c=e.target.closest?.('.game-card');if(!c)return;e.preventDefault();e.stopImmediatePropagation();const u=urlOf(c);recordPlay(c);if(u)openGame(u,titleOf(c));renderRails()},{capture:true});
 document.addEventListener('keydown',e=>{if(e.key==='Escape'&&$('categorySidebar')?.classList.contains('open'))closeCategories()},{capture:true});
}
function sync(){cards().forEach(c=>{if(blockedTitle(c))c.remove();else wire(c)});repairImages();decorate();renderRails();applyView();const s=$('status');if(s&&cards().length)s.textContent=cards().length+' games ready'}
async function loadGlobalTrending(){
  ...
  try{
    ...
    const rows=await r.json(),map=Object.create(null);
    (Array.isArray(rows)?rows:[]).forEach(x=>{const k=String(x?.game_key||'');const n=Number(x?.play_count||0);if(k&&Number.isFinite(n)&&n>0)map[k]=n});
    const previous=globalTrending.counts;
    let changed=Object.keys(map).length!==Object.keys(previous).length;
    if(!changed)for(const k of Object.keys(map)){if(Number(previous[k]||0)!==Number(map[k]||0)){changed=true;break}}
    globalTrending.counts=map;
    globalTrending.updatedAt=new Date().toISOString();
    globalTrending.ready=true;
    globalTrending.failed=false;
    if(!changed){globalTrending.loading=false;updateTrendRailState();return;}
  }catch(_){globalTrending.failed=true}
  globalTrending.loading=false;updateTrendRailState();decorate();renderRails();applyView();
}
function start(){
  ...
  loadGlobalTrending();
  setInterval(loadGlobalTrending,60000);
  loadLegacyIntoGrid().then(()=>{sync();loadGlobalTrending()}).catch(()=>{sync();loadGlobalTrending()});
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',start,{once:true});else start();
})();
</script>'''
